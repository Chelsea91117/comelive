...
urlpatterns = [
    ...
    path('bookings/<int:pk>/', BookingRetrieveUpdateGenericView.as_view(), name='booking-update'),
    # изменение статуса объявления для владельцев и арендаторов
    path('bookings/ads/<int:ad_id>/', BookedDatesListGenericView.as_view(), name='property-booked-dates'),
    # лист забронированных дат для всех пользователей
    path('reviews/<int:ad_id>/', ReviewListGenericView.as_view(), name='ad-reviews-list'),
