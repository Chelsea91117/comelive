...
import datetime
from comelive.models import User, Ad, Booking, Review
...
class AdSerializer(serializers.ModelSerializer):
    ...
    def validate_state(self, value):
        if not re.match(r"^([a-zA-Z ]{2,50})$", value):
            raise serializers.ValidationError(
                "The state must be represented by alphabetic characters"
            )
        return value
    ...
    def validate_city(self, value):
        if not re.match(r"^([a-zA-Z ]{2,50})$", value):
            raise serializers.ValidationError(
                "The city must be represented by alphabetic characters"
            )
        return value
...
class BookingCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_ad(self, ad):
        if not ad.is_active:
            raise serializers.ValidationError(
                "This ad is not active, you cannot book it."
            )
        return ad
    ...
    def validate(self, data):
        ...
        if ad and start_date and end_date:
            ...
            if overlapping_bookings.exists():
                raise serializers.ValidationError(
                    {"start_date and end_date": "The dates are already taken"}
                )
...
class BookingUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if user.is_renter and new_status != "Rejected":
            raise serializers.ValidationError(
                {"status": "Renter can only update status to Rejected"}
            )

        if user.is_landlord and new_status == "Pending":
            raise serializers.ValidationError(
                {"status": "Landlord can only update status to Rejected or Confirmed"}
            )
...
class ReviewCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        booking = Booking.objects.filter(
            ad=ad, renter=user, status="Confirmed", end_date__lte=datetime.date.today()
        ).exists()

        if not booking:
            raise serializers.ValidationError(
                {"ad": "You can leave a review only after staying at this property."}
            )
